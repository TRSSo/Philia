...
export type ClassMethod<C, T extends Command | Middleware | Event | Schedule> = Omit<T, "method" | "type" | "scene"> & {
	method: {
		[K in keyof C]: C[K] extends (T["method"] extends (...args: infer Args) => infer R ? (...args: Args extends [unknown, ...infer Rest] ? Rest : []) => R : never) ? K : never;
	}[keyof C];
};
/** 类插件 */
export interface ClassPlugin<T, E extends Philia.Event.Event> {
	...
	scene?: E["scene"];
	/** 命令组 */
	command?: E extends Philia.Event.Message ? ClassMethod<T, Command<E>> | ClassMethod<T, Command<E>>[] : never;
	/** 中间件组 */
	...
	/** 开启插件 */
	start?: {
		[K in keyof T]: T[K] extends () => void | Promise<void> ? K : never;
	}[keyof T];
	/** 关闭插件 */
...
/** 单命令插件 */
export interface SingleCommand<T extends Philia.Event.Message = Philia.Event.Message> extends ACommand<T> {
	cmd: string;
	/**
	 * 命令方法
...
/** 多命令插件 */
export interface MultiCommand<T extends Philia.Event.Message = Philia.Event.Message> extends ACommand<T> {
	cmd: string[];
	/**
	 * 命令方法
	 * @param cmd 匹配到的命令
...
/** 正则命令插件 */
export interface RegExpCommand<T extends Philia.Event.Message = Philia.Event.Message> extends ACommand<T> {
	cmd: RegExp;
	/**
	 * 命令方法
...
/** 命令插件 */
export type Command<T extends Philia.Event.Message = Philia.Event.Message> = SingleCommand<T> | MultiCommand<T> | RegExpCommand<T>;
/** 中间件插件 */
