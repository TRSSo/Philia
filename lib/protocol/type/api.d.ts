...
export type IAPI<T extends Record<keyof T, {
	request: unknown;
	response: unknown;
}>> = {
	[K in keyof T]: (data: T[K]["request"], client?: Client) => T[K]["response"] | Promise<T[K]["response"]>;
};
