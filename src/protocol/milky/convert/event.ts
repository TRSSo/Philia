...
export default class Event {
    ...
    switch (data.data.message_scene) {
      ...
      case "friend":
        event = {
          ...
          user: await this.impl.handle.getUserInfo({ id: String(data.data.peer_id) }),
        }
      ...
      case "group":
        event = {
          ...
          group: await this.impl.handle.getGroupInfo({ id: String(data.data.peer_id) }),
        }
  ...
  async FriendRequest(data: Milky.Struct.FriendRequest) {
    const event: Philia.Event.UserRequest = {
      ...
      time: data.time,
      scene: "user_add",
      user: await this.impl.handle.getUserInfo({ id: String(data.initiator_id) }),
      state: data.state,
      reason: data.comment,
    }
    return event
  ...
  async GroupRequest(data: Milky.Struct.GroupRequest) {
    ...
    switch (data.request_type) {
      case "invite":
        ...
        event.target = await this.impl.handle.getUserInfo({ id: String(data.invitee_id) })
        break
      case "join":
  ...
  async friend_nudge(data: Milky.Event.FriendNudge) {
    const event: Philia.Event.UserPoke = {
      ...
      time: data.time,
      scene: "user_poke",
      user: await this.impl.handle.getUserInfo({ id: String(data.data.user_id) }),
    }
  ...
  async group_admin_change(data: Milky.Event.GroupAdminChange) {
    const event: Philia.Event.GroupMemberInfo = {
      ...
      user: (await this.impl.handle.getGroupMemberArray({ id: String(data.data.group_id) })).find(
        i => i.role === "owner",
      )!,
      ...
      group: await this.impl.handle.getGroupInfo({ id: String(data.data.group_id) }),
