...
/** 消息事件基类 */
export interface AEMessage extends IEvent {
  type: "message"
  user: IUser
  message: IMessage
  /** 消息摘要 */
...
/** 用户消息事件 */
export interface IEMUser extends AEMessage {
  scene: "user"
  /** 如果是自己发送给用户，则存在该字段 */
  is_self?: true
}
...
/** 群消息事件 */
export interface IEMGroup extends AEMessage {
  scene: "group"
  group: IGroup
}

export type IEMessage = IEMUser | IEMGroup
...
export default class Event {
  ...
  event = new Proxy({
    ...
    get(target, prop: string) {
      ...
      const array = Object.keys(this).filter(i => i.startsWith(`${prop}.`)) as (keyof typeof target)[]
      if (array.length)
        return target[array[Math.floor(Math.random()*array.length)]]
    },
  })
